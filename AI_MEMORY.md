...
- ✅ Система миграций схемы (`src/database/migrations.ts`) — текущая версия схемы: **4**
...
- ✅ `dgWho` и `dgWhen` — встроенные свойства Task для делегирования (migration v4, `dg_who`/`dg_when` в SQLite)
...
**Тестов: 327, все зелёные.** Запуск: `npm test`

### Последний коммит в репо

`3541d5f` — feat: add dgWho and dgWhen built-in properties, migration v4 (327 tests)

> Репо чистое, незакоммиченных изменений нет.
